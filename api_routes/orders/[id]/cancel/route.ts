...
export async function POST(
  ...
  try {
    ...
    // Ownership check: must be buyer or farmer associated with order
    if (
      order.buyer_id !== user!.uid &&
      order.buyer_id !== dbUserId &&
      order.farmer_id !== user!.uid &&
      order.farmer_id !== dbUserId
    ) {
    ...
    if (order.status === 'CANCELLED' || order.status === 'DELIVERED') {
      return NextResponse.json({
        success: false,
        error: `Order is already in final status '${order.status}'`,
      }, { status: 400 });
    }
